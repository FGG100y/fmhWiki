...
**取舍**：
- 每个工具调用建一个子 turn —— 会污染会话时间线（一次用户指令产生多个 turn，编辑历史树膨胀），且前端时间线无需展示中间步。**排除**。
...
链路 1 文生图 / 链路 2 图生图 / 链路 3 版本分支 —— 配置全默认（`AGENTIC_EDIT_ENABLED=false`、不带 `options.mode`），按 `TODOs.md` 原断言执行，行为与原实现完全一致。（会话级 undo/redo 已废弃，用户通过编辑历史树实现版本切换。）
