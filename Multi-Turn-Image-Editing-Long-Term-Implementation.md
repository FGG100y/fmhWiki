...
### 3.4 与 LangChain deepAgent 的区别与优势

先说结论：`deepAgent` 更适合“开放式任务求解”，而本文方案更适合“可回放、可追踪、可回退”的多轮修图产品。

| 维度 | LangChain deepAgent（典型） | 本文方案（LangGraph + 版本树 + Job） |
|---|---|---|
| 核心范式 | 以 Agent 自主规划和工具调用为中心 | 以状态机和版本状态管理为中心 |
| 流程形态 | 动态探索路径，灵活但不确定性高 | 固定主链路 + 条件分支，行为可预测 |
| 历史管理 | 主要是消息上下文和中间轨迹 | 一等公民版本树（`parent_turn_id`） |
| 回退/分支 | 需额外工程封装 | 架构原生支持 undo/redo/switch/branch |
| 可观测性 | 偏 Agent 执行日志 | 面向产品指标：turn、job、model call 全链路 |
| 质量闭环 | 常见做法是“失败再让 Agent 试” | 显式 Visual QA + 可控重试策略 |
| 适配多轮修图 | 能做，但需补大量产品化状态层 | 天然匹配“当前图、输入图、mask、参数、输出图” |

为什么本文方案在多轮修图里更有优势：

- 用户需求本质是“版本控制 + 局部编辑控制”，不是“开放问答推理深度”。
- 业务风险点是“改坏了哪里、能不能回到上一版、能不能追责这次模型调用”，这些都依赖结构化状态，而不是 Agent 自由规划。
- 线上运行要可控成本和可控延迟，固定节点（intent → rewrite → route → run → QA）更容易做 SLA、熔断和降级。
- 合规和审核通常要求可审计链路；turn / job / model call 三层记录比纯 Agent 轨迹更容易满足审计需求。

这并不表示 `deepAgent` 没价值。更合理的用法是：

- 把 `deepAgent` 用在“离线策略优化”或“复杂 prompt/工具策略搜索”；
- 在线主流程仍使用本文这种确定性工作流承接生产流量。
