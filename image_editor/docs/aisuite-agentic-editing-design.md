...
本设计面向 `drawAgent`（`feat/local-model-router` 分支，HEAD `ef64921e70e6`，设计时间 2026-08-04）。当前系统是一个 **确定性管道式** 的图像编辑应用：
...
**与具体后端的解耦原则**：规划工具依赖的是"候选工具注册表 + 优先级路由"这一抽象，而非任何具体工具。Moebius / LaMa / doubao 都只是注册在 `ToolMeta` 下的候选实现，带 `is_local` / `priority` 属性；移除或新增任一后端只需调整注册与优先级，**不改变本设计的任何接口**（规划工具、步迹、示范注入、降级逻辑均与具体后端无关）。未来即便完全去掉 Moebius 本地角色，本设计无需改动。
...
**与既有机制的关系**：示范只作用于**提示层**，不改动路由与工具层——因为 `select_tool` 的优先级已决定候选工具选择（本地候选优先，由 `ToolMeta.priority` 表达），工具偏好本质上已由路由体现，示范补充的是路由表达不出的**措辞、顺序与 mask 习惯**。因此示范注入不新增任何工具，也不改变 `resolve_task_type` 语义，与核心问题一/二的薄封装、图像工作记忆正交。
...
新概念 **LLM 接入层（LLM access layer）**：基于 aisuite `Client` 的、供应商无关的模型调用抽象。规划循环经 `Runner.run`（带工具调用），视觉 QA 经 `chat.completions`（多模态问答）；两者都以 `provider:model` 寻址，落地为 `llm/aisuite_client.py`。
...
| agentic_edit → aisuite | 直接依赖：`Agent` + `Runner` 作为标准工具循环引擎 |
...
| max_turns | `AGENTIC_MAX_TURNS` | `5` | aisuite Runner 轮次上限 |
...
async def run_agentic_edit(state: ImageEditState) -> dict:
    """agentic_edit 节点：用规划模型 + aisuite Runner 执行多步编辑循环。

    流程：取示范记录（若启用）→ 渲染注入系统提示 → 组装 Agent + Runner → 跑循环。
    返回值：
...
if config.agentic.demo_enabled:                        # 工作记忆：会话内示范（在后）
    ...
    if demo:
        system_prompt += "\n\n" + render_demo_prompt(demo)

client = aisuite.Client(provider_configs=build_provider_configs())   # LLM 接入层
agent = aisuite.Agent(name="image-editor", model=config.agentic.planner_model,
                      instructions=system_prompt,
                      tools=[generate_image, edit_image, inspect_image])
result = await aisuite.Runner.run(agent, user_instruction,
                                  client=client, max_turns=config.agentic.max_turns)
```

提取步迹：从 `result.tool_events`（每个工具调用的名称/参数/状态）与 `result.messages` 中还原产物，组装 `list[AgentStep]`。
...
def build_provider_configs() -> dict[str, dict]:
    ...
    例：ARK_API_KEY + ARK_BASE_URL → {"openai": {"api_key": ..., "base_url": ...}}；
    其他供应商（google/ollama/...）按各自环境变量注册。新增供应商只加环境变量。
    """
...
**关系总结**：`build_provider_configs` 是"供应商配置注册表"的实现，`get_client` 提供共享 `Client`，`vision_answer` 是视觉 QA 的统一入口——三者构成 **LLM 接入层**：规划循环经 `Runner.run` 消费同一 `Client`，视觉 QA 经 `vision_answer`，都以 `provider:model` 寻址，供应商可切换而不改业务代码。
...
**最小变更**：新增一个**执行模式选择控件**（确定性 / 自动 / agentic），随创建 turn 的请求经 `CreateTurnRequest.options.mode` 发送——API 请求/响应形状均不变（`options` 已存在），属纯前端增量。`TurnDetailResponse` 可（可选）暴露 `agent_steps` 供后续展示，v1 前端不消费。

## 5. 一致性校验

**概念一致性**：全文统一使用"规划模型 / 规划工具 / 图像工作记忆 / 代理步迹 / 最终产物 / 决策模式 / 示范记录 / 示范注入 / LLM 接入层 / 供应商配置注册表 / 记忆分层 / 偏好档案"十二个概念；"规划工具"与底层"图片工具（ImageToolRegistry 成员）"严格区分，不与"任务类型（generate/edit/inpaint）"混用；"示范记录"是 turn 历史的投影，不与"代理步迹"（agentic 自身执行记录）混为一谈；"工作记忆（示范）"与"长期记忆（偏好档案）"分属不同聚合范围（会话内 vs 跨会话），不混用。

**接口完备性**：
- 设计逻辑 2.5 的 4 个降级触发点均有对应实现路径：调用异常 → 异常捕获转 degraded；轮次/调用上限 → Runner 返回后检查计数；连续失败 → 步迹中失败计数；无最终产物 → 检查 `output_image_id`。
...
- **前端模式控件**：UI 呈现 确定性 / 自动 / agentic 三态，切换后请求携带对应 `options.mode`。
